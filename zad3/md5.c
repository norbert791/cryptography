...
static void md5_alter_transform(MD5_CTX* ctx, const BYTE data[]) {
  WORD a, b, c, d, m[16], i, j;
...
void md5_alter_update(MD5_CTX* ctx, const BYTE data[], size_t len) {
  size_t i;
  ...
  for (i = 0; i < len; ++i) {
    ...
    ctx->datalen++;
    if (ctx->datalen == 64) {
      md5_alter_transform(ctx, ctx->data);
      ctx->bitlen += 512;
      ctx->datalen = 0;
    }
  }
